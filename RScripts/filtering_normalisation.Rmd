...
ggplot(beta_long, aes(x = Beta, color = Sample)) +
  ...
  labs(title = "Density Plot of Beta Values After DASEN Normalization")
```

# Remove cross-hybridizing probes (AFTER normalization)

```{r}
# Load Zhou probe masking data
print("Loading Zhou probe masking data...")
ZhouProbeMasking <- gzcon(
  url(paste("https://github.com/zhou-lab/InfiniumAnnotationV1/raw/main/Anno/",
            "EPIC/archive/202209/EPIC.hg19.manifest.tsv.gz", sep=""))) %>%
  read_tsv()

# Identify cross-reactive probes from Zhou's paper
# "Comprehensive characterization, annotation and innovative use of Infinium 
# DNA methylation BeadChip probes"
cross_hybrid <- ZhouProbeMasking[ZhouProbeMasking$MASK_general %in% "TRUE",] %$%
  probeID
...
cat("Total probes removed from original:", nrow(dasen_result$beta) - nrow(beta_filtered), "\n")
cat("Percentage of original probes retained:", round(nrow(beta_filtered)/nrow(dasen_result$beta) * 100, 2), "%\n")
```
...
original_probes <- nrow(dasen_result$beta)
...
# Filter annotations to match final probe set
...
sample_metadata <- phenoData_pe[match(colnames(beta_filtered), phenoData_pe$Basename), ]

# Add any missing sample information from targets if needed
if(any(is.na(sample_metadata$Basename))) {
  missing_samples <- colnames(beta_filtered)[is.na(sample_metadata$Basename)]
  additional_info <- targets_pe[targets_pe$Basename %in% missing_samples, ]
  # Handle missing samples appropriately based on your data structure
}

# 4. Create filtering summary for reproducibility
filtering_log <- list(
  original_probes = nrow(dasen_result$beta),
  cross_hybrid_removed = length(cross_hybrid[cross_hybrid %in% rownames(dasen_result$beta)]),
  detection_pval_removed = length(missing_probes_sex_aware),
  beadcount_removed = length(low_bead_probes_sex_aware),
  total_removed = nrow(dasen_result$beta) - nrow(beta_filtered),
  ...
  removed_probe_lists = list(
    ...
    all_removed_probes = c(cross_hybrid[cross_hybrid %in% rownames(dasen_result$beta)], 
                          all_failed_probes)
  )
)
...
cat("- Beta and M-values same dimensions:", identical(dim(beta_filtered), dim(M_filtered)), "\n")
cat("- Sample metadata matches beta columns:", identical(colnames(beta_filtered), sample_metadata$Basename), "\n")
cat("- Probe annotations match beta rows:", identical(rownames(beta_filtered), rownames(probe_annotations_filtered)), "\n")
...
cat("- Sample metadata missing:", sum(is.na(sample_metadata$Outcome)), "outcomes,", 
    sum(is.na(sample_metadata$Sex)), "sex\n")

print("Enhanced data saving complete! Ready for differential methylation analysis.")

# ===== QUICK START GUIDE FOR DOWNSTREAM ANALYSIS =====
cat("\n=== QUICK START FOR DIFFERENTIAL ANALYSIS ===\n")
cat("To load data for analysis:\n")
cat("load('", file.path(outdir, "processed_methylation_data_complete.RData"), "')\n", sep="")
cat("\nKey objects available:\n")
cat("- beta_filtered: Beta values for reporting/visualization\n")
cat("- M_filtered: M-values for statistical testing\n") 
cat("- sample_metadata: Sample information with outcomes\n")
cat("- probe_annotations_filtered: Probe genomic annotations\n")
cat("- design_matrix: Ready-to-use design matrix (if created)\n")
```